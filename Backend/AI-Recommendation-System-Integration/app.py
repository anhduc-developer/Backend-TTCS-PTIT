...
os.environ["LANGCHAIN_TRACING_V2"] = os.getenv("LANGCHAIN_TRACING_V2") or get_secret("LANGCHAIN_TRACING_V2")
...
try:
    ...
    resume_text = ""
    st.sidebar.error(
        "Không thể đọc nội dung resume. Vui lòng kiểm tra lại file PDF và thử lại."
    )
...
def execute_chat_conversation(user_input, graph, resume_text=""):
    callback_handler_instance = initialize_callback_handler(st.container())
    callback_handler = callback_handler_instance
    try:
        messages = []
        if resume_text:
            messages.append(HumanMessage(content=f"Resume content:\n\n{resume_text}"))
        messages.extend(list(message_history.messages))
...
with input_section:
    ...
    if submit_query_button:
        if not uploaded_document:
            ...
            chat_output = execute_chat_conversation(user_input_query, flow_graph, resume_text)
