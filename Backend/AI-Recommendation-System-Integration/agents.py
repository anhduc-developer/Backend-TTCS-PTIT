...
def create_tool_agent(llm: ChatOpenAI, tools: list, system_prompt: str):
    """
    Creates a tool-enabled LangChain agent using the current langchain API.

    Args:
        llm : LLM to be used to create the agent.
        tools (list): A list of tool callables or tool objects.
        system_prompt (str): System prompt for the agent.

    Returns:
        CompiledStateGraph: The compiled LangChain agent graph.
    """
...
def job_search_node(state):
    ...
    """
    llm = init_chat_model(**state["config"])
    search_agent = create_tool_agent(
...
def resume_analyzer_node(state):
    ...
    """
    llm = init_chat_model(**state["config"])
    analyzer_agent = create_tool_agent(
        llm,
        [ResumeExtractorTool(state.get("resume_text", ""))],
        get_analyzer_agent_prompt_template(),
    )
...
def cover_letter_generator_node(state):
    ...
    """
    llm = init_chat_model(**state["config"])
    generator_agent = create_tool_agent(
        llm,
        [
            generate_letter_for_specific_job(state.get("resume_text", "")),
            save_cover_letter_for_specific_job,
            ResumeExtractorTool(state.get("resume_text", "")),
        ],
        get_generator_agent_prompt_template(),
